...
class SafeBoxGUI:
    ...
    global engine, extract_metadata
    engine = CryptoEngine()
    extract_metadata = MetadataHandler.detach
    ...
    def execute_encrypt(self):
        ...
        try:
            result = engine.encrypt(self.encrypt_file_path.get(), self.encrypt_password.get(), self.encrypt_output_path.get())
    ...
    def execute_decrypt(self):
        ...
        try:
            result = engine.decrypt(self.decrypt_file_path.get(), self.decrypt_password.get(), output_path)
            if isinstance(result, tuple):
                message, *err = result
                color = "red" if "[-]" in message else "green" if "[+]" in message else "blue"
                self.status.config(text=message, fg=color)
